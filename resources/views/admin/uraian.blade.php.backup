<x-admin-layout>
    <div class="bg-white p-4 mb-4 border-2 border-white rounded-lg">
        <h3 class="text-gray-700 text-2xl font-medium">Uraian Resiko</h3>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">{{ $uraian->uraian }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="
                            @if($uraian->status == 'On Progress')
                                bg-yellow-500 text-yellow-800
                            @elseif($uraian->status == 'Accepted')
                                bg-green-500 text-green-800
                            @elseif($uraian->status == 'Rejected')
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="inline-flex space-x-4 justify-center items-center">
                            <a href="javascript:void(0);"
                               onclick="openModal('{{ route('admin.uraian.update', $uraian->id) }}', '{{ $uraian->uraian }}', '{{ $uraian->status }}')"
                            ...
                            <form action="{{ route('admin.uraian.destroy', $uraian->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
    ...
    <script>
    function openModal(action, uraian, status) {
    document.getElementById('editForm').action = action;
    document.getElementById('uraianInput').value = uraian;
