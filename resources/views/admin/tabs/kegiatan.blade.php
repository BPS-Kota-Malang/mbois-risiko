    ...
    <section class="bg-white dark:bg-white">
        <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            <div
                        ...
                        @foreach ($activities as $activity)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    {{ $activity->name }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    {{ $activity->subteam->name ?? '-' }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    {{ $activity->subteam->timProject->name ?? '-' }}</td>
                                    @if (auth()->check() && auth()->user()->hasRole('admin'))
                                    <td class="px-6 py-4 text-center whitespace-no-wrap border-b border-gray-200">
                                        <div class="inline-flex space-x-4">
                                            <button
                                                onclick="openEditTeamProjectModal('{{ route('admin.activity.update', $activity->id) }}', '{{ $activity->name }}', '{{ $activity->deskripsi ?? '' }}')"
                                            ...
                                            <form action="{{ route('admin.activity.destroy', $activity->id) }}" method="POST" class="inline"
                ...
                <div id="activityAddModal" class="fixed inset-0 flex items-center justify-center hidden">
                    ...
                    <div
                        ...
                        <form action="{{ route('admin.activity.store') }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label class="block mb-2 text-gray-700" for="name">Nama Kegiatan</label>
                            ...
                            @if (auth()->check() && auth()->user()->hasRole('admin'))
                                <div class="mb-4">
                                    <label class="block mb-2 text-gray-700" for="name">Pilih Sub Team</label>
                            ...
                            @else
                                <input type="hidden" name="subteam" value="{{ auth()->user()->subteam }}" />
                            @endif
                ...
                <div id="timProjectEditModal" class="fixed inset-0 flex items-center justify-center hidden">
                    ...
                    <div
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button onclick="toggleActivityModal('edit')"
                                class="ml-4 text-2xl text-gray-500">&times;</button>
                        </div>
...
<script>
    function toggleActivityModal(action) {
        const modalId = action === 'tambah' ? 'activityAddModal' : 'timProjectEditModal';
    ...
    function openEditTeamProjectModal(url, namaTeam, deskripsi) {
        ...
        toggleActivityModal('timProjectEditModal');
    }
</script>
