                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-center">
                                    <div class="inline-flex space-x-4 justify-center">
                                        ...
                                        <form id="deleteLvldampak-{{ $leveldampak->id }}"
                                            action="{{ route('admin.leveldampak.destroy', $leveldampak->id) }}"
                                            method="POST" style="display:none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <button class="bg-red-500 text-white px-3 py-1 rounded cursor-pointer"
                                            onclick="confirmDelete(`{{ $leveldampak->id }}`)">Delete</button>
                                        </form>
                                    </div>
                                </td>
                                @endif
    ...
    function confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('deleteLvldampak-' + id).submit();
                }
            });
        }
</script>
